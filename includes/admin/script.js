...
( function( $ ) {
    $( document ).ready(
        function() {
            $selector = $( document ).find( 'div#docket-cache' );
            $selector.find( 'a#refresh' ).on(
                ...
                function( e ) {
                    ...
                    var sort = $selector.find( 'select#sort' ).children( 'option:selected' ).val();
            ...
            var spinner = function() {
                $( window ).on(
                    'beforeunload',
                    function() {
                        $( document ).find( '#docket-cache-overlay' ).css( 'display', 'block' );
                        setTimeout(
                            function() {
                                $( document ).find( '#docket-cache-spinner' ).css( 'display', 'inline-block' );
                            },
                            750
                        );
                    }
            ...
            $selector.find( 'p.submit' ).find( 'a.button' ).on(
                'click',
                function() {
                    spinner();
                }
            ...
            $selector.find( 'p.submit' ).find( 'select' ).on(
                'change',
                function() {
                    spinner();
                }
